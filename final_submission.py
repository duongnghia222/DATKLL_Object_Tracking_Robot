...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if keyboard_control:
        ...
        circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1,
                                   minDist=rows / 8, param1=50, param2=60, minRadius=30, maxRadius=50)
        prev_circle = None
        ...
        if contours_found:
            ...
            if right_bound < cx:
                if car_in_active:
                    turn_right(init_default_speed - 10)
            elif left_bound > cx:
                if car_in_active:
                    turn_left(init_default_speed  - 10)
            else:
        ...
        if key == ord("l"):
            stop()
            car_in_active = not car_in_active
            if car_in_active:
                print('car is in active')
            else:
                print('car is not in active')
    if key == ord("r"):
